...
if save_MSD
    ...
    MSD_T = table('Size',[0,8],'VariableTypes',{'cell','double','double','double','double','double','double','double'},...
        'VariableNames',{'ImageName','DiffCoeff','ExposureTime','SNR','background','noise','new_track_id','MSD'});
    writetable(MSD_T,fullfile(MSD_savePath,'MSD.csv'),'WriteRowNames',true);
else
    MSD_savePath = img_pair_saveDir; % path to save MSD results
    MSD_T = table('Size',[0,7],'VariableTypes',{'cell','double','double','double','double','double','double'},...
        'VariableNames',{'ImageName','DiffCoeff','ExposureTime','SNR','background','noise','new_track_id'});
...
for iSPT = 1:length(simSPT_file)
    ...
    for idT = 1:length(target_exposure)
            ...
            if dataHeight >= target_exposure(idT)
                for j = 1:windowSize:dataHeight
                    if j+target_exposure(idT)-1 < dataHeight % not exceed the data height
                        ...
                        if total_subtrack_idx > max_imgpair_array(iSPT)
        ...
        if length(now_track_cell) < max_imgpair_array(iSPT)
        ...
        for iter = 1:length(now_track_cell)
            ...
            if min(pix_track_x) < 0 || min(pix_track_y) < 0 || max(pix_track_x) > 320 || max(pix_track_y) > 320 || OutOfRangeError
                ...
                mask_pix_merge_z = uint8(mask_pix_merge_z); % convert from double to uint8
                ...
                imwrite(noisy_pix_merge_z,fullfile(img_pair_saveDir,'imgs',sprintf('%s_TrackID%012d_D%s_SNR%d_dT%02dms_good.tif','Pixelized',now_track.trajecotry_newIdx(1),simSPT_D(iSPT),round(SNR),target_exposure(idT))));
                % imwrite(mask_pix_merge_z,fullfile(img_pair_saveDir,'masks',sprintf('%s_TrackID%012d_D%s_SNR%d_dT%02dms_good_mask.gif','Pixelized',now_track.trajecotry_newIdx(1),simSPT_D(iSPT),round(SNR),target_exposure(idT))));
                
                % File Name that record SNR in table later
                pix_filename_good{iter,1} = sprintf('%s_TrackID%012d_D%s_SNR%d_dT%02dms_good','Pixelized',now_track.trajecotry_newIdx(1),simSPT_D(iSPT),round(SNR),target_exposure(idT));
                %=========    create ground truth of motion blur/mask with good SNR ===============%
                ...
                if save_MSD
                    ...
                    ImageName = pix_filename_good(iter,1);
                    % ImageName = {sprintf('%s_TrackID%012d_D%s_dT%02dms_good.tif','Pixelized',now_track.trajecotry_newIdx(1),simSPT_D(iSPT),target_exposure(idT))};
                    new_line = table(ImageName,DiffCoeff(iSPT),target_exposure(idT),...
                        ...
                        now_track.trajecotry_newIdx(1),temp_simSPT_track_msd.msd{1}(2,2)); %MSD at 1*dT
