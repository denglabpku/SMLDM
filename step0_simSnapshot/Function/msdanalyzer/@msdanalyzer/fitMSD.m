...
% Yiwen Modification:
% We let origin take part in fitting and give the origin weight=(1dt weight +1) 
% also , we use clip_factor=0.25, which follow the paper:
% Goulian, Mark, and Sanford M. Simon. 2000. “Tracking Single Proteins within Cells.” Biophysical Journal 79 (4): 2188–98. https://doi.org/10.1016/S0006-3495(00)76467-8.
...
for i_spot = 1 : n_spots
    ...
    % Clip data, take the first one dt = 0，which assume no localization error
    if clip_factor < 1
        t_limit = 1 : round(numel(t) * clip_factor);
    else
        t_limit = 1 : min(round(clip_factor), numel(t));
    end
    t = t(t_limit);
    y = y(t_limit);
    w = w(t_limit);
