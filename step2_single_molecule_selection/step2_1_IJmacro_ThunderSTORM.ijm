...
for (j = 0; j < input_dirs.length; j++) {
    dir_processing = input_dirs[j];
    dir_saving = output_dirs[j];

    // Create output folder if it doesn't exist
    if (!File.exists(dir_saving)) {
        File.makeDirectory(dir_saving);
    }
    ...
    for (i = 0; i < list.length; i++) {
        if (endsWith(list[i], ".nd2")) {
            ...
            run("Bio-Formats Importer",
                "open=" + img_dir + " color_mode=Default rois_import=[ROI manager] split_channels view=Hyperstack stack_order=XYCZT");
